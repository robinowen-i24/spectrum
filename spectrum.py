...
def run(fid, minimum_nrg, incident_nrg, cutoff, mapme, spread, offset, include_list, exclude_list, plot_indv):
    ...
    scale_dict, params_dict = get_scale_dict(poss_emis_dict, vortex_nrg_axis, \
                                                sum_spec, spread, offset, cutoff, exclude_list, include_list)
    ...
    try_fit = True
    if try_fit == True: 
        token = ft.try_fit(scale_dict, poss_emis_dict, params_dict, vortex_nrg_axis, sum_spec) 
